...
public class NcCurveAnimation : NcEffectBehaviour
{
	// Class ------------------------------------------------------------------------
...
// 				range.xMax = Mathf.Max(range.yMax, m_AniCurve[n].time);
	...
	{
		if (enabled == false || NgObject.IsActive(gameObject) == false)
			return -1;
		if ((0 < m_fDurationTime && (m_fStartTime == 0 || GetEngineTime() - m_fStartTime - m_fDelayTime < m_fDurationTime)))
			return 1;
		return 0;
	}
...
// 		Debug.Log("NcCurveAnimation.Update " + transform.parent.name);
	...
	void UpdateAnimation(float fElapsedRate)
	{
		...
		{
			// AutoDestruct
			if (m_bAutoDestruct && 1 < m_fElapsedRate)
			{
