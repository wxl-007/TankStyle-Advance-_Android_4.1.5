...
// 			return false;
		...
		{
			if (m_MeshUVs[n].x == 0)		value[n].x = uv.x;
			if (m_MeshUVs[n].y == 0)		value[n].y = uv.y;
			if (m_MeshUVs[n].x == 1.0f)		value[n].x = uv.x+uv.width;
			if (m_MeshUVs[n].y == 1.0f)		value[n].y = uv.y+uv.height;
