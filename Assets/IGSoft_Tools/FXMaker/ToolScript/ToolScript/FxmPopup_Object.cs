...
public class FxmPopup_Object : FxmPopup
{
	...
	void winPopup(int id)
	{
		...
		if (m_SelObjectType == FXMakerHierarchy.OBJECT_TYPE.OBJECT_GAMEOBJECT)
		{
			...
			// MoveToParent
			if (FXMakerLayout.GUIButton(FXMakerLayout.GetInnerVerticalRect(baseRect, nButtonCount, nDrawCount, 2), GetHelpContent("MoveToParent"), (m_SelectedTransform != transOriginalRoot && (m_SelectedTransform.parent != transOriginalRoot) || (m_SelectedTransform.parent == transOriginalRoot && transOriginalRoot.GetChildCount()==1))))
			{
				if (m_SelectedTransform.parent == transOriginalRoot && transOriginalRoot.GetChildCount()==1)
				{
