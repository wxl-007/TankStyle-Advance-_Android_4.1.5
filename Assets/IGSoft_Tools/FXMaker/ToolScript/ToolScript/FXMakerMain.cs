...
public class FXMakerMain : MonoBehaviour
{
	...
	public		enum TOOLDIR_TYPE		{DEFAULTSHADERMATERIALS=0, TEMPPREVIEWTEXTURE, BACKGROUNDPREFABS, BACKGROUNDRESOURCES, PROJECTS, SCRIPTS, SCREENSHOTSAVEDIR, SPRITE_TOOL, SPRITE_USER};
...
// 			NgAssembly.GetPropertis(FXMakerHierarchy.inst.GetSelectedGameObject().renderer);
		...
		if (0 < m_nStartThumbCapture)
		{
			...
			{
				if (m_nToolIndex == 0)
					FXMakerCapture.EndSaveBackThumb();
				else FXMakerCapture.EndSaveEffectThumb();
				SetActiveTool(m_nToolIndex);
