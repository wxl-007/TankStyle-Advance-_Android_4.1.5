...
public class FXMakerEffect : MonoBehaviour
{
	...
	public			GameObject			m_DefaultEffectPrefab;
...
// 				FXMakerMain.inst.ToggleGlobalLangSkin(false);
				...
				if (FXMakerMain.GetPrevWindowFocus() == (int)FXMakerLayout.WINDOWID.EFFECT_LIST && FXMakerMain.GetWindowFocus() != (int)FXMakerLayout.WINDOWID.EFFECT_LIST)
					RenameCurrentPrefab(m_EditingName);
			}
		}
...
// 		FXMakerMain.inst.ToggleGlobalLangSkin(false);
	...
	void ShowRightMenu(int nObjectIndex, bool bSelected)
	{
		...
		FxmPopupManager.inst.ShowMenuPopup(new string[]{"PingPrefab", "Copy", "Cut", "Paste", "-", "New", "Delete", "Clone", "Thumb", "BuildSprite", "Export", "ExportAll"},
