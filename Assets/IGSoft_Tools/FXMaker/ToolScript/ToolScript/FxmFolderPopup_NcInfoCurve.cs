...
// 			GUI.enabled = false;
	...
	void ShowRightMenu(int nObjectIndex)
	{
		...
		FxmPopupManager.inst.ShowMenuPopup(new string[]{"Delete"});
	}
	...
	string GetCurveInfo(NcCurveAnimation.NcInfoCurve curveInfo, string subDir)
	{
		if (curveInfo != null)
		{
			if (m_bOnlyCurve)
			{
				...
				option2 = (curveInfo.m_ApplyType == NcCurveAnimation.NcInfoCurve.APPLY_TYPE.COLOR) ? "To Color: "+curveInfo.m_ToColor.ToString()+"\n- Recursively: "+curveInfo.m_bRecursively.ToString() : "Value Scale: "+curveInfo.m_fValueScale.ToString();
